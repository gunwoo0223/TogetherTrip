$(document).ready(function() {
	...
	$('#sendNumBtn').on('click',function(){
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			data : 'uEmail='+$('#uEmail').val()+'&uName='+$('#uName').val(),
			url : 'mail.do',
			success : function(res){
				$('#sendNum').val(res.sendNum);
				alert(res.msg);
			}		
		});
	});
	...
	//인증번호 입력후 확인버튼 눌렀을 때
	$('#chkNumBtn').on('click',function(){
		$.ajax({
			type : 'POST',
			dataType : 'json',
			data : 'chkNum='+$('#chkNum').val()+'&sendNum='+$('#sendNum').val(),
			url : 'checkNum.do',
			success : function(res){
				alert(res.msg);
				if(res.check == 0){
	...
	$('#join').on('click', function() {
		...
		alert('['+$('#uName').val()+']님 회원가입이 완료되셨습니다. 환영합니다!');
		return true;
	});
});
