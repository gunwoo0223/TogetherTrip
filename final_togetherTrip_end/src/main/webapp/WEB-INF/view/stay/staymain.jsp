   ...
   <h1 class="page-header" style="font-size: 15px; text-align: -webkit-right;">총 게시물의 수 :${cnt}</h1>
         ...
         <div class="row">
            <c:forEach items="${sList}" var="sdto">
               <c:forEach items="${sdto.sImage}" var="sidto">
                  <div class="col-md-3 portfolio-item">
                     <a href="detail.do?s_num=${sdto.s_num}"> 
                     <img class="img-responsive" src="${sidto.s_thumbnail}" style="width: 700px; height: 205px;">
                     </a>
                     <h4>
                        <a href="detail.do?s_num=${sdto.s_num}" style="text-align: center;">${sdto.s_title}</a>
                     </h4>
                     <h5>${sdto.s_tel}</h5>
                     <h5>${sdto.s_loc}</h5>
                  </div>
	...
	<div class="col-lg-12">
		<ul class="pagination">
			...
			<c:if test="${pgdto.startPage > 1}">
				<li><a href="stay.do?currentPage=${pgdto.startPage-pgdto.blockPage}">&laquo;</a></li>
			</c:if>
			<!-- 이전 출력 끝 -->
		
			<!-- 페이지 출력 시작 -->
			<c:forEach var="i" begin="${pgdto.startPage}" end="${pgdto.endPage}">
				<c:url var="currPage" value="stay.do">
			...
			<c:if test="${pgdto.totalPage>pgdto.endPage}">
				<li><a href="stay.do?currentPage=${pgdto.startPage+pgdto.blockPage}">&raquo;</a></li>
			</c:if>
			<!--  다음 출력 끝 -->
		</ul>
	</div>
