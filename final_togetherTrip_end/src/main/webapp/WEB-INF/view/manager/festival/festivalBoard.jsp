  ...
  <section id="container" >
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-angle-right"></i>Festival</h3>
		  		<div class="row mt">
			  		<div class="col-lg-12">
                            ...
                            <table class="table table-condensed">
                             ...
                             <c:forEach items="${fList}" var="fdto">
                              <tr>
                                  <td>${fdto.f_num}</td>
                                  <td>${fdto.m_id}</td>
                                  <td><a href="festivalUpdateWrite.do?f_num=${fdto.f_num}">${fdto.f_title}</a></td>
                                  <td>${fdto.f_reg_date}</td>
                                  <td>${fdto.f_read_count}</td>
                                  <td><button type="button" onclick="festivalDeleteBtn(${fdto.f_num});">삭제</button></td>
                              </tr>
                              </c:forEach>
                              </tbody>
                          </table>
	...
	<c:if test="${pgdto.startPage > 1}">
		<li><a href="festivalBoard.do?currentPage=${pgdto.startPage-pgdto.blockPage}">&laquo;</a></li>
	</c:if>
	<!-- 이전 출력 끝 -->

	<!-- 페이지 출력 시작 -->
	<c:forEach var="i" begin="${pgdto.startPage}" end="${pgdto.endPage}">
		<c:url var="currPage" value="festivalBoard.do">
	...
	<c:if test="${pgdto.totalPage>pgdto.endPage}">
		<li><a href="festivalBoard.do?currentPage=${pgdto.startPage+pgdto.blockPage}">&raquo;</a></li>
	</c:if>
	<!--  다음 출력 끝 -->
