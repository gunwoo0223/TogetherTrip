  ...
  <section id="container" >
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-angle-right"></i> Place</h3>
		  		<div class="row mt">
			  		<div class="col-lg-12">
                            ...
                            <table class="table table-condensed">
                             ...
                             <c:forEach items="${pList}" var="pdto">
                              <tr>
                                  <td>${pdto.p_num}</td>
                                  <td>${pdto.m_id}</td>
                                  <td><a href="placeUpdateWrite.do?p_num=${pdto.p_num}">${pdto.p_title}</a></td>
                                  <td>${pdto.p_reg_date}</td>
                                  <td>${pdto.p_read_count}</td>
                                  <td><button type="button" onclick="placeDeleteBtn(${pdto.p_num});">삭제</button></td>
                              </tr>
                              </c:forEach>
                              </tbody>
                          </table>
	...
	<c:if test="${pgdto.startPage > 1}">
		<li><a href="placeBoard.do?currentPage=${pgdto.startPage-pgdto.blockPage}">&laquo;</a></li>
	</c:if>
	<!-- 이전 출력 끝 -->

	<!-- 페이지 출력 시작 -->
	<c:forEach var="i" begin="${pgdto.startPage}" end="${pgdto.endPage}">
		<c:url var="currPage" value="placeBoard.do">
	...
	<c:if test="${pgdto.totalPage>pgdto.endPage}">
		<li><a href="placeBoard.do?currentPage=${pgdto.startPage+pgdto.blockPage}">&raquo;</a></li>
	</c:if>
	<!--  다음 출력 끝 -->
