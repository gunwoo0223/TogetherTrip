  ...
  <section id="container" >
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-angle-right"></i>Restaurant</h3>
		  		<div class="row mt">
			  		<div class="col-lg-12">
                            ...
                            <table class="table table-condensed">
                             ...
                             <c:forEach items="${rList}" var="rdto">
                              <tr>
                                  <td>${rdto.r_num}</td>
                                  <td>${rdto.m_id}</td>
                                  <td><a href="restaurantUpdateWrite.do?r_num=${rdto.r_num}">${rdto.r_title}</a></td>
                                  <td>${rdto.r_reg_date}</td>
                                  <td>${rdto.r_read_count}</td>
                                  <td><button type="button" onclick="restaurantDeleteBtn(${rdto.r_num});">삭제</button></td>
                              </tr>
                              </c:forEach>
                              </tbody>
                          </table>
	...
	<c:if test="${pgdto.startPage > 1}">
		<li><a href="restaurantBoard.do?currentPage=${pgdto.startPage-pgdto.blockPage}">&laquo;</a></li>
	</c:if>
	<!-- 이전 출력 끝 -->

	<!-- 페이지 출력 시작 -->
	<c:forEach var="i" begin="${pgdto.startPage}" end="${pgdto.endPage}">
		<c:url var="currPage" value="restaurantBoard.do">
	...
	<c:if test="${pgdto.totalPage>pgdto.endPage}">
		<li><a href="restaurantBoard.do?currentPage=${pgdto.startPage+pgdto.blockPage}">&raquo;</a></li>
	</c:if>
	<!--  다음 출력 끝 -->
